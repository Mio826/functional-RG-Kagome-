...
class StandardKagomeFormFactorLibrary:
    """
    Patch-space recognition library for common kagome form factors.

    This library is intentionally practical rather than mathematically complete.
    It mixes:
      1) generic lattice harmonics (s/p/d/f), and
      2) kagome-paper-inspired f-wave templates from Kiesel-Platt-Thomale PRL 2013.

    The output labels should be read as "this mode most resembles ...".
    """
    ...
    def _paper_f_templates(self) -> Dict[str, np.ndarray]:
        kx = self.k[:, 0]
        ky = self.k[:, 1]
        rt3 = np.sqrt(3.0)
        return {
            "f_paper_ab": np.sin(1.5 * kx + 0.5 * rt3 * ky),
            "f_paper_bc": np.sin(1.5 * kx - 0.5 * rt3 * ky),
            "f_paper_ac": np.sin(rt3 * ky),
        }
    ...
    def build(
        ...
        *,
        ...
        include_d: bool = True,
        include_f: bool = True,
        include_paper_f: bool = True,
    ) -> List[FormFactorCandidate]:
        th = self._angles()
        c2 = np.cos(2.0 * th)
        s2 = np.sin(2.0 * th)
        ...
        if include_f:
            add("f_x", self._sin_sum(c2), "B_like", "odd", "f", {"source": "generic_bond_harmonic"})
            add("f_y", self._sin_sum(s2), "B_like", "odd", "f", {"source": "generic_bond_harmonic"})

        if include_paper_f:
            for name, values in self._paper_f_templates().items():
                add(name, values, "paper_f", "odd", "f", {"source": "Kiesel2013 Eq(5)"})
...
class FormFactorAnalyzer:
    def __init__(self, patchset, candidates: Sequence[FormFactorCandidate]):
        ...
        for cand in self.candidates:
            if np.asarray(cand.values).shape != (self.Npatch,):
                raise ValueError(
                    f"Candidate {cand.name!r} has shape {np.asarray(cand.values).shape}, expected ({self.Npatch},)."
                )
    ...
    def from_standard_kagome(
        ...
        *,
        ...
        lib = StandardKagomeFormFactorLibrary(patchset, delta_vectors=delta_vectors)
        candidates = lib.build(
            include_constant=include_constant,
            include_extended=include_extended,
            include_p=include_p,
            include_d=include_d,
            include_f=include_f,
            include_paper_f=include_paper_f,
        )
    ...
    def analyze_mode(self, mode: np.ndarray, *, rank: int = 1, eigenvalue: complex = np.nan) -> KernelModeAnalysis:
        mode_fixed = fix_global_phase(mode)
        matches: List[ModeMatch] = []
        grouped: Dict[str, float] = {}

        for cand in self.candidates:
            basis = cand.normalized_values()
            ov = np.vdot(basis, mode_fixed)
    ...
    def analyze_kernel(
        ...
        *,
        ...
        if M.shape != (self.Npatch, self.Npatch):
            raise ValueError(f"Kernel matrix has shape {M.shape}, but patchset has Npatch={self.Npatch}.")
...
def summarize_analysis(analysis: KernelFormFactorAnalysis, *, top_matches: int = 4) -> str:
    ...
    for mode in analysis.modes:
        ...
        lines.append("  grouped weights: " + ", ".join([f"{k}={v:.3f}" for k, v in mode.grouped_weights.items()]))
        lines.append("  top matches: " + ", ".join([f"{m.name} ({m.weight:.3f})" for m in mode.matches[:top_matches]]))
    return "\n".join(lines)
...
def print_top_matches(mode_analysis: KernelModeAnalysis, *, top_matches: int = 6) -> None:
    ...
    for m in mode_analysis.matches[:top_matches]:
        print(
            f"  {m.name:14s} family={m.family:>2s} irrep={m.irrep:>8s} parity={m.parity:>4s} "
            f"weight={m.weight:.6f} overlap={m.overlap}"
        )
