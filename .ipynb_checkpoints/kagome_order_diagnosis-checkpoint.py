...
class KagomeOrderDiagnoser:
    """
    Paper-oriented Kagome order diagnosis layer built on top of channel decomposition
    and the coarse OrderRecognizer.

    IMPORTANT DESIGN CHOICE:
    This layer must remain open to genuinely new orders. Therefore it has two outputs:
      1) a coarse symmetry-level label from OrderRecognizer
      2) a paper-level label among {FM, PI, cBO, sBO, f-SC} *only if* the
         paper-specific template support is sufficiently strong.

    Otherwise it returns:
        paper_label = "unclassified"
        recognition_status = "novel_candidate"

    This allows the user to inspect the mode manually instead of forcing every instability
    into one of the five orders discussed in the paper.
    """
    ...
    # ---------- approximate tensor reconstruction ----------
    def reconstruct_mode_tensor(self, kernel: Any, mode: ModeAnalysis) -> np.ndarray:
        """
        Return tensor basis with shape (Npatch, Norb, Norb, subspace_dim).
        We reconstruct from the *column* patch basis because columns represent the
        incoming bilinear in the channel kernel definitions.
        """
    ...
    def _template_tensors(self, kernel: Any) -> Dict[str, np.ndarray]:
        ...
        # PI templates: diagonal / same-sublattice, Q=0, d-wave Eq. (3) of the paper.
        ...
        # f-SC templates: Eq. (5) of the paper, symmetric off-diagonal sublattice pairs.
        ...
        # BO templates: finite-Q, off-diagonal pair structure using a cyclic Q-to-pair convention.
    ...
    def _final_label(
        ...
        if qnorm < self.q_zero_tol and name.startswith("ph"):
            pi_score = 0.0
            for t in template_scores:
                if t.name in {"PI_dx2y2", "PI_dxy"}:
                    pi_score = max(pi_score, t.score)
        ...
        if qnorm >= self.q_zero_tol and name.startswith("ph"):
            bo_score = 0.0
            for t in template_scores:
                if t.name == "BO_finiteQ":
                    bo_score = max(bo_score, t.score)
            offdiag_weight = float(np.sum(np.abs(pair_weights - np.diag(np.diag(pair_weights)))))
            if top.name == "BO_finiteQ" or (mode.dominant_label == "finiteQ_cos_sin" and pair_weights.sum() > 0 and offdiag_weight > np.trace(pair_weights)):
        ...
        if name.startswith("pp"):
            fscore = 0.0
            for t in template_scores:
                if t.name.startswith("fSC_"):
                    fscore = max(fscore, t.score)
    ...
    def diagnose_kernel(self, kernel: Any, *, sort_by: str = "abs") -> KagomeOrderDiagnosis:
        ...
        mode = analyses[0]
        tensor_basis = self.reconstruct_mode_tensor(kernel, mode)
        pair_weights, dominant_pair, same_w, inter_w = self._pair_weight_summary(tensor_basis)
        templates = self._template_tensors(kernel)
        template_scores = self._project_tensor_basis(tensor_basis, templates)
        paper_label, paper_score, recognition_status, notes = self._final_label(kernel, mode, pair_weights, template_scores)
        notes = list(mode.notes) + notes
        if dominant_pair is not None:
            lbl = (
                self._pair_labels.get(dominant_pair[0], str(dominant_pair[0])),
                self._pair_labels.get(dominant_pair[1], str(dominant_pair[1])),
            )
