...
def compute_pp_vertex_contribution_sz0(
    ...
    *,
    ...
    p3: int,
    p4: int,
    internal_cache: Mapping[Tuple[str, str], Dict[str, np.ndarray]],
) -> complex:
    """
    Minimal S_z=0 pp loop contribution.

    This remains a scaffold until the full SU(2) reduction is re-derived.
    At present we only keep the internal (up,dn) sector.
    """
    ...
    for a in range(len(partner)):
        ...
        if b < 0 or abs(weights[a]) == 0:
            continue
        acc += weights[a] * vfn(p1, p2, a, b) * vfn(a, b, p3, p4)
    return complex(acc)
...
def compute_phd_vertex_contribution_sz0(
    ...
    *,
    ...
    """
    Minimal S_z=0 direct ph loop contribution.
...
def compute_phc_vertex_contribution_sz0(
    ...
    *,
    ...
    """
    Minimal S_z=0 crossed ph loop contribution.
    ...
    for a in range(len(partner)):
        ...
        if b < 0 or abs(weights[a]) == 0:
            continue
        term1 = vfn(p1, a, p4, b) * vfn(b, p2, a, p3)
        term2 = vfn(p1, b, p4, a) * vfn(a, p2, b, p3)
        acc += weights[a] * (term1 + term2)
    return complex(acc)
