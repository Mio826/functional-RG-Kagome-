...
class FRGFlowSolver:
    ...
    def _precompute_transfer_tables(self) -> None:
        ...
        # Directed particle-hole transfer tables.
        # For a source leg k_src and a target leg k_tgt:
        #   plus table  stores iq(Q) for Q = k_tgt - k_src  (target = source + Q)
        #   minus table stores iq(Q) for Q = k_src - k_tgt  (target = source - Q)
    ...
    def _precompute_shift_maps(self) -> None:
        ...
        for iq, Q in enumerate(self.phc_grid.q_list):
            for s1 in self._spins:
                for s2 in self._spins:
                    # phc must use its own transfer grid / q-index tables for both k+Q and k-Q.
                    # Even when phd/phc grids happen to coincide numerically, mixing the helper here
                    # makes the routing convention inconsistent with the rest of the phc machinery.
                    self._phc_kplus[(iq, s1, s2)] = self._partner_map_phc_from_iq(
                        iq,
                        first_spin=s1,
                        second_spin=s2,
                        Q=Q,
                    ...
                    self._phc_kminus[(iq, s1, s2)] = self._partner_map_phc_from_iq(
                        iq,
                        first_spin=s1,
                        second_spin=s2,
                        Q=Q,
    ...
    def compute_vertex_rhs(self, T: float) -> Dict[SpinBlock, np.ndarray]:
        ...
        for key in self.spin_blocks:
            ...
            qphd = self._phd_q_index_plus[(s1, s3)]
            qphc = self._phc_q_index_plus[(s2, s3)]
            rhs_block = rhs[key]
