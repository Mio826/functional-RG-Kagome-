...
from frg_geometry_shared import (
    SharedGeometry,
...
class FRGFlowSolver:
    def __init__(
        ...
        *,
        ...
        if shared_geometry is None:
            shared_geometry = build_shared_geometry(
                patchsets,
                spin_blocks=self.spin_blocks,
                pp_Qs=pp_Qs,
                ph_Qs=ph_Qs,
                phc_Qs=phc_Qs,
                q_merge_tol_red=q_merge_tol_red,
                q_key_decimals=q_key_decimals,
            )
        self._shared_geometry = shared_geometry

        self.pp_grid = shared_geometry.pp_grid
        self.phd_grid = shared_geometry.phd_grid
        self.phc_grid = shared_geometry.phc_grid
        ...
        self._spins = list(shared_geometry.spins)
        self._internal_spin_pairs = tuple(shared_geometry.internal_spin_pairs)
        self.Npatch = int(shared_geometry.Npatch)
        self._pp_q_index = shared_geometry.pp_q_index
        self._phd_q_index = shared_geometry.phd_q_index
        self._phc_q_index = shared_geometry.phc_q_index
        self._pp_qminus = shared_geometry.pp_qminus
        self._ph_kplus = shared_geometry.ph_kplus
        self._phc_kplus = shared_geometry.phc_kplus
        self._phc_kminus = shared_geometry.phc_kminus
        self._closure_map = shared_geometry.closure_map
    ...
    def shared_geometry(self) -> SharedGeometry:
    ...
    def _estimate_bare_vertex_norm(self) -> float:
        vals = [float(np.max(np.abs(block.data))) for block in self.state.gamma_blocks.values()]
        return max(max(vals), 1e-14) if vals else 1e-14
    ...
    def compute_vertex_rhs(self, T: float) -> Dict[SpinBlock, np.ndarray]:
        ...
        def gval(key: SpinBlock, p1: int, p2: int, p3: int, p4: int) -> complex:
            ...
            if data is None:
                return zero
            if int(block_p4[key][p1, p2, p3]) != int(p4):
                return zero
