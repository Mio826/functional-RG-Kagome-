...
class FRGFlowSolver:
    def __init__(
        ...
        *,
        ...
        self._patch_k = {s: _canonicalize_patch_k_array(np.asarray([p.k_cart for p in patchset_for_spin(self.patchsets, s).patches], dtype=float), self.patchsets) for s in self._spins}
    ...
    def _build_q_level_internal_caches(self, T: float):
        ...
        for iq, Q in enumerate(self.pp_grid.q_list):
            ...
            pp_internal_by_iq[iq] = build_pp_internal_cache_vec(self.patchsets, cfg, shift_cache=shift_cache)
        for iq, Q in enumerate(self.phd_grid.q_list):
            shift_cache = {(sa, sb): self._ph_kplus[(iq, sa, sb)] for sa, sb in available_internal_spin_pairs(self.patchsets)}
            ph_internal_by_iq[iq] = build_ph_internal_cache_vec(self.patchsets, cfg, shift_cache=shift_cache)
        for iq, Q in enumerate(self.phc_grid.q_list):
            shift_cache = {(sa, sb): self._phc_kplus[(iq, sa, sb)] for sa, sb in available_internal_spin_pairs(self.patchsets)}
            phc_internal_by_iq[iq] = build_ph_internal_cache_vec(self.patchsets, cfg, shift_cache=shift_cache)
        return cfg, pp_internal_by_iq, ph_internal_by_iq, phc_internal_by_iq

    def compute_vertex_rhs(self, T: float) -> Dict[SpinBlock, np.ndarray]:
        gamma = self._fast_gamma
        _, pp_internal_by_iq, ph_internal_by_iq, phc_internal_by_iq = self._build_q_level_internal_caches(T)
        rhs: Dict[SpinBlock, np.ndarray] = {key: np.zeros((self.Npatch, self.Npatch, self.Npatch), dtype=complex) for key in self.spin_blocks}

        for key in self.spin_blocks:
            s1, s2, s3, s4 = key
            block = self.state.gamma_blocks[key]
            p4_idx = block.p4_index
            for p1 in range(self.Npatch):
                for p2 in range(self.Npatch):
                    iq_pp = int(self._pp_q_index[(s1, s2)][p1, p2])
                    Q_pp = self.pp_grid.q_list[iq_pp]
                    pp_internal = pp_internal_by_iq[iq_pp]
                    for p3 in range(self.Npatch):
                        p4 = int(p4_idx[p1, p2, p3])
                        if p4 < 0:
                            continue
                        val_pp = 0.0 + 0.0j
                        for (sa, sb), info in pp_internal.items():
                            ...
                            for a in range(len(partner)):
                                ...
                                if b < 0 or abs(weights[a]) == 0:
                                    continue
                                val_pp += weights[a] * gamma(p1, s1, p2, s2, a, sa, b, sb) * gamma(a, sa, b, sb, p3, s3, p4, s4)

                        iq_phd = int(self._phd_q_index[(s3, s1)][p3, p1])
                        Q_phd = self.phd_grid.q_list[iq_phd]
                        phd_internal = ph_internal_by_iq[iq_phd]
                        val_phd = 0.0 + 0.0j
                        for (sa, sb), info in phd_internal.items():
                            partner = np.asarray(info["partner"], dtype=int)
                            weights = np.asarray(info["weights"], dtype=complex)
                            for a in range(len(partner)):
                                b = int(partner[a])
                                if b < 0 or abs(weights[a]) == 0:
                                    continue
                                term1 = gamma(p1, s1, a, sa, p3, s3, b, sb) * gamma(b, sb, p2, s2, a, sa, p4, s4)
                                term2 = gamma(p1, s1, b, sb, p3, s3, a, sa) * gamma(a, sa, p2, s2, b, sb, p4, s4)
                                val_phd -= weights[a] * (term1 + term2)

                        iq_phc = int(self._phc_q_index[(s3, s2)][p3, p2])
                        Q_phc = self.phc_grid.q_list[iq_phc]
                        phc_internal = phc_internal_by_iq[iq_phc]
                        val_phc = 0.0 + 0.0j
                        for (sa, sb), info in phc_internal.items():
                            partner = np.asarray(info["partner"], dtype=int)
                            weights = np.asarray(info["weights"], dtype=complex)
                            for a in range(len(partner)):
                                b = int(partner[a])
                                if b < 0 or abs(weights[a]) == 0:
                                    continue
                                term1 = gamma(p1, s1, a, sa, p4, s4, b, sb) * gamma(b, sb, p2, s2, a, sa, p3, s3)
                                term2 = gamma(p1, s1, b, sb, p4, s4, a, sa) * gamma(a, sa, p2, s2, b, sb, p3, s3)
                                val_phc += weights[a] * (term1 + term2)

                        rhs[key][p1, p2, p3] = val_pp + val_phd + val_phc
        return rhs
